...
def create_test_cases():
    ...
    cv2.rectangle(
        image1, (50, 50), (350, 150), (200, 200, 200), -1
    )  # Light gray watermark
    cv2.putText(
        image1, "PROTECT", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 3
    )
    ...
    cv2.putText(
        image2, "GRAY", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (80, 80, 80), 3
    )  # Gray text
    cv2.putText(
        image2, "TEXT", (120, 140), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (80, 80, 80), 3
    )
    ...
    cv2.rectangle(
        image3, (50, 50), (350, 150), (210, 210, 210), -1
    )  # Very light watermark
    cv2.putText(
        image3, "ELECTRONIC", (80, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 0), 2
    )
    ...
    cv2.putText(
        image4, "SCANNED", (90, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (20, 20, 20), 2
    )
    cv2.putText(
        image4, "TEXT", (130, 140), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (20, 20, 20), 2
    )
...
def evaluate_text_protection(image, mask, text_region=None):
    ...
    if text_region is None:
        ...
        text_region = mask[
            text_region[1] : text_region[3], text_region[0] : text_region[2]
        ]
...
def test_protection_improvement():
    ...
    for case_name, image, watermark_color in test_cases:
        ...
        results.append({"case_name": case_name, "evaluation": evaluation})
...
def compare_with_baseline():
    ...
    improvement_pct = (
        (improvement / eval_without["text_pixels"]) * 100
        if eval_without["text_pixels"] > 0
        else 0
    )
    ...
    print(
        f"  Protection effectiveness: {'EXCELLENT' if improvement_pct > 50 else 'GOOD' if improvement_pct > 25 else 'FAIR'}"
    )
...
def create_summary_report(results):
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            f"   Text protection: {eval_data['quality']} ({eval_data['text_pixels']} pixels)"
        )

    # Overall assessment
    excellent_count = sum(
        1 for r in results if r["evaluation"]["quality"] == "EXCELLENT"
    )
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("CONCLUSION:")
    print("The 'Protect First, Refine Second' approach with improved")
    print(f"text protection achieves {improvement_pct:.1f}% reduction in text")
    print("pixels being included in the watermark mask.")
    print("=" * 60)
