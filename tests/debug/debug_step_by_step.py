...
def manual_protection_process():
    ...
    final_protected = cv2.bitwise_and(protected_mask, cv2.bitwise_not(text_protect_mask))
    print(f"After text protection: {np.count_nonzero(final_protected)}")
    print(f"Text pixels removed: {np.count_nonzero(protected_mask) - np.count_nonzero(final_protected)}")
    ...
    opened_mask = cv2.morphologyEx(final_protected, cv2.MORPH_OPEN, kernel, iterations=1)
    ...
    visualization = np.vstack([
        ...
        np.hstack([cv2.cvtColor(protected_mask, cv2.COLOR_GRAY2BGR), cv2.cvtColor(text_protect_mask, cv2.COLOR_GRAY2BGR)]),
        np.hstack([cv2.cvtColor(final_protected, cv2.COLOR_GRAY2BGR), cv2.cvtColor(closed_mask, cv2.COLOR_GRAY2BGR)])
    ])
...
def test_different_text_colors():
    ...
    for i, color in enumerate(text_colors):
        ...
        gray_val = color[0]
        cv2.putText(image, f"Text{gray_val}", (50 + i*80, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    ...
    for i, (gray_val, protected) in enumerate(results):
        ...
        cv2.putText(viz_image, f"{gray_val}", (50 + i*80, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        cv2.putText(viz_image, status, (50 + i*80, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255) if not protected else (0, 255, 0), 1)
