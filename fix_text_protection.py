...
def test_improved_protection():
    ...
    text_protection = improved_text_protection(gray, watermark_mask, gray_threshold=150, expand_pixels=3)
    ...
    print(f"Text pixels removed: {np.count_nonzero(watermark_mask) - np.count_nonzero(protected_mask)}")
...
def modify_watermark_detector():
    ...
    with open('src/pdf_watermark_removal/watermark_detector.py', 'r') as f:
        content = f.read()
...
def test_modified_detector():
    ...
    print(f"Text protection: {'PASS' if text_pixels < 100 else 'FAIL'}")  # Allow some edge pixels
