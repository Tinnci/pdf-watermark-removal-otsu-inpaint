...
def debug_protection_steps():
    ...
    print(f"   - Range: {target_gray - color_tolerance} - {target_gray + color_tolerance}")
    ...
    text_protect = cv2.morphologyEx(text_protect, cv2.MORPH_OPEN, kernel_protect, iterations=1)
    
    print(f"\n2. Text protection mask:")
    print(f"   - Threshold: 150 (protects 0-150)")
    print(f"   - Non-zero pixels: {np.count_nonzero(text_protect)}")
    print(f"   - Text pixels that should be protected: {np.count_nonzero(text_protect)}")
    
    # Check specific text area
    text_area = gray[80:120, 100:300]  # Approximate text region
    print(f"   - Text area gray values: min={text_area.min()}, max={text_area.max()}, mean={text_area.mean():.1f}")
    ...
    print(f"   - Text pixels removed: {np.count_nonzero(color_mask) - np.count_nonzero(protected_mask)}")
    ...
    print(f"   - Background pixels removed: {np.count_nonzero(protected_mask) - np.count_nonzero(final_protected)}")
...
def test_text_color_ranges():
    ...
    cv2.imwrite("text_color_test.png", np.hstack([image, cv2.cvtColor(text_protect, cv2.COLOR_GRAY2BGR)]))
